 ...
 */
export function populateRoundMomentum(momentum) {
    const container = document.getElementById('round-momentum-container');
    if (!container || momentum.length === 0) {
        if (container)
            container.innerHTML = '<p class="no-data">No round momentum data available</p>';
        return;
    }
 ...
 */
export function createInlineMomentumChart(round) {
    const width = 700;
    const height = 120;
    const padding = 30;
    ...
    // Scale functions
    const xScale = (holeNum) => padding + ((holeNum - 1) / (round.holes.length - 1)) * chartWidth;
    const yScale = (score) => padding + chartHeight / 2 - (score / range) * (chartHeight / 2);
    // Create path
 ...
 */
function createMomentumChart(round) {
    const width = 600;
    const height = 150;
    const padding = 30;
    const chartWidth = width - 2 * padding;
    ...
    const points = round.holes.map(h => `${xScale(h.holeNumber)},${yScale(h.cumulativeScoreToPar)}`).join(' ');
    // Create path string
    const pathData = round.holes.map((h, i) => `${i === 0 ? 'M' : 'L'} ${xScale(h.holeNumber)} ${yScale(h.cumulativeScoreToPar)}`).join(' ');
    let svg = `
 ...
 */
export function populateCourseLearningCurves(curves) {
    const container = document.getElementById('course-learning-container');
    if (!container || curves.length === 0) {
        if (container)
            container.innerHTML = '<p class="no-data">Need 3+ rounds at a course to show learning curve</p>';
        return;
    }
 ...
 */
function createLearningChart(curve) {
    const width = 400;
    const height = 120;
    const padding = 30;
    const chartWidth = width - 2 * padding;
    ...
    const xScale = (roundNum) => padding + ((roundNum - 1) / (curve.totalRounds - 1)) * chartWidth;
    const yScale = (score) => padding + ((maxScore - score + 1) / range) * chartHeight;
    const pathData = curve.progression.map((p, i) => `${i === 0 ? 'M' : 'L'} ${xScale(p.roundNumber)} ${yScale(p.scoreToPar)}`).join(' ');
    let svg = `
