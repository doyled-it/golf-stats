 ...
 */
export function populatePuttingPerformance(putting) {
 ...
 */
function createPuttingTrendChart(trendData) {
    ...
    container.innerHTML = createSVGLineChart(rollingAvg, labels, 'Avg Putts/Hole (5-round avg)', '#9C27B0');
}
/**
 * Populates birdie conversion section
 */
export function populateBirdieConversion(birdie) {
 ...
 */
export function populateApproachShotAccuracy(accuracy) {
 ...
 */
export function populateStrokesGained(sg) {
 ...
 */
function getStrokesGainedColor(value) {
    if (value >= 2)
        return '#00C853'; // Dark green - excellent
    if (value >= 1)
        return '#4CAF50'; // Green - very good
    if (value >= 0)
        return '#81C784'; // Light green - good
    if (value >= -2)
        return '#FFB74D'; // Light orange - slightly below scratch
    if (value >= -4)
        return '#FF9800'; // Orange - moderately below scratch
    if (value >= -6)
        return '#FF5722'; // Dark orange - well below scratch
 ...
 */
export function populatePerformanceCorrelations(correlations) {
