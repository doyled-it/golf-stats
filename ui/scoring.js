 ...
 */
export function populateScoringTrends(rounds, roundMomentum) {
 ...
 */
export function populateScoringHeatmap(heatmap) {
    ...
    function getHeatmapColor(scoreToPar) {
        if (scoreToPar <= 0)
            return '#00441b'; // Dark green - excellent
        if (scoreToPar <= 4)
            return '#006d2c'; // Green - great
        if (scoreToPar <= 8)
            return '#31a354'; // Light green - good
        if (scoreToPar <= 12)
            return '#74c476'; // Pale green - average
        if (scoreToPar <= 16)
            return '#fdae6b'; // Orange - below average
        if (scoreToPar <= 20)
            return '#fd8d3c'; // Dark orange - poor
 ...
 */
export function populateHandicapHistory(history) {
    const container = document.getElementById('handicap-chart');
    if (!container)
        return;
    container.innerHTML = createSVGLineChart(history.map(h => h.handicapIndex), history.map(h => formatDate(h.date)), 'Handicap Index', '#4CAF50');
}
/**
 * Creates score history chart
 */
export function populateScoreHistory(rounds) {
    ...
    container.innerHTML = createSVGLineChart(normalizedScores, recentRounds.map(r => formatDate(r.date)), 'Score to Par (18-hole equiv.)', '#2196F3');
}
/**
 * Populates hole type performance
 */
export function populateHoleTypePerformance(holeTypes) {
