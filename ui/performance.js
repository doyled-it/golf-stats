 ...
 */
export function populatePersonalRecords(records) {
 ...
 */
export function populateMomentum(momentum) {
 ...
 */
export function populateRecentForm(recentForm, recentRounds) {
 ...
 */
function populateFormIndicator(containerId, form, rounds) {
    const container = document.getElementById(containerId);
    if (!container || !form || !rounds || rounds.length === 0)
        return;
 ...
 */
function createSparkline(scores, improving) {
    const width = 200;
    const height = 40;
    const padding = 2;
