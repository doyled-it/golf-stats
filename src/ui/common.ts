 ...
 */
export function getScoreClass(scoreToPar: number): string {
    if (scoreToPar <= -3) return 'score-excellent';
    if (scoreToPar <= -1) return 'score-good';
    if (scoreToPar <= 2) return 'score-average';
    if (scoreToPar <= 5) return 'score-poor';
    return 'score-bad';
}
 ...
 */
export function createSVGLineChart(
    data: number[],
    labels: string[],
    yAxisLabel: string,
    color: string
