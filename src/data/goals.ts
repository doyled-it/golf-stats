 ...
 */
export function calculateGoalsData(
  ...
  const sortedHistory = [...handicapHistory].sort((a, b) =>
  ...
  const recentHistory = sortedHistory.filter(h =>
...
function generateBurndownData(
  ...
  const points: GoalBurndownPoint[] = [];
  ...
  while (currentDate <= maxDeadline) {
    ...
    let actualHandicap: number | undefined;
    ...
    let goalHandicap: number | undefined;
