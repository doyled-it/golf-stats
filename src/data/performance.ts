 ...
 */
export function calculatePersonalRecords(rounds: Round[]): PersonalRecords {
  ...
  if (rounds.length > 0) {
    const bestRound = rounds.reduce((best, round) =>
      round.differential < best.differential ? round : best
    );
  ...
  const sortedRoundsWithHoles = roundsWithHoles.sort((a, b) =>
 ...
 */
export function calculateMomentum(rounds: Round[]): MomentumIndicators {
  const momentum: MomentumIndicators = {};

  if (rounds.length === 0) return momentum;

  // Track performance after birdies, pars, bogeys
  let afterBirdieScores: number[] = [];
  let afterBirdieParOrBetter = 0;
  let afterParScores: number[] = [];
  let afterParParOrBetter = 0;
  let afterBogeyScores: number[] = [];
  let afterBogeyParOrBetter = 0;

  rounds.forEach(round => {
    if (!round.holes || round.holes.length < 2) return;
 ...
 */
export function calculateCourseDifficulty(rounds: Round[]): { mostDifficult?: any; easiest?: any } {
  ...
  const mostDifficult = validCourses.reduce((max, course) =>
    course.difficultyScore > max.difficultyScore ? course : max
  );

  // Find course with lowest difficulty score (easiest)
  const easiest = validCourses.reduce((min, course) =>
    course.difficultyScore < min.difficultyScore ? course : min
  );
