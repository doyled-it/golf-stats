 ...
 */
export function calculateBirdieConversion(rounds: Round[]): BirdieConversion | undefined {
 ...
 */
export function calculatePuttingPerformance(rounds: Round[]): PuttingPerformance | undefined {
  if (rounds.length === 0) return undefined;
  ...
  const puttingTrend: PuttingTrendData[] = [];
 ...
 */
export function calculateStrokesGained(rounds: Round[], handicapIndex: number = 12): StrokesGained | undefined {
  if (rounds.length === 0) return undefined;
  ...
  if (roundsWithData === 0) return undefined;
 ...
 */
export function calculatePerformanceCorrelations(rounds: Round[]): PerformanceCorrelation[] | undefined {
  if (rounds.length < 3) return undefined; // Need minimum data for correlations
  ...
  if (roundStats.length < 3) return undefined;
  ...
  correlations.forEach(corr => {
    ...
    if (absCorr > 0.6) corr.impact = 'high';
    else if (absCorr > 0.3) corr.impact = 'medium';
    else corr.impact = 'low';
  });
