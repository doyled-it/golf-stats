 ...
 */
export function calculateRecentForm(recentRounds: Round[], overallAveragePerHole: number): RecentForm | undefined {
  if (recentRounds.length === 0) return undefined;
 ...
 */
export function calculateScoringHeatmap(rounds: Round[]): ScoringHeatmap {
  ...
  const roundsByDate = new Map<string, Round[]>();
 ...
 */
export function calculateHoleDifficulty(rounds: Round[]): HoleDifficulty[] | undefined {
  if (rounds.length === 0) return undefined;

  // Group holes by hole number
  const holeMap = new Map<number, { scores: number[], par: number }>();
  ...
  const difficulties: HoleDifficulty[] = [];
