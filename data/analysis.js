 ...
 */
export function calculateRoundMomentum(rounds, limit = 10) {
    const momentum = [];
 ...
 */
export function calculateCourseLearningCurves(rounds) {
    ...
    courseRounds.forEach((courseRoundsList, key) => {
        ...
        if (courseRoundsList.length < 3)
            return;
        // Sort by date
        const sortedRounds = courseRoundsList.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
        const firstRound = sortedRounds[0];
        const latestRound = sortedRounds[sortedRounds.length - 1];
        // Find best score
        const bestRound = sortedRounds.reduce((best, round) => round.totalScore < best.totalScore ? round : best);
        const bestRoundIndex = sortedRounds.indexOf(bestRound);
        // Calculate progression points
        const progression = sortedRounds.map((round, index) => ({
            date: round.date,
            roundNumber: index + 1,
