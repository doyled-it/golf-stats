 ...
 */
import { calculateRecentForm, calculateScoringHeatmap, calculateHoleDifficulty } from './scoring.js';
import { calculateBirdieConversion, calculatePuttingPerformance, calculateStrokesGained, calculatePerformanceCorrelations } from './shots.js';
import { calculatePersonalRecords, calculateMomentum, calculateCourseDifficulty } from './performance.js';
import { calculateRoundMomentum, calculateCourseLearningCurves } from './analysis.js';
import { calculateGoalsData } from './goals.js';
/**
 ...
 */
export function transformGHINScore(ghinScore, index) {
    ...
    const holes = ghinScore.hole_details?.map(hole => ({
...
async function loadGHINData() {
    ...
    const sortedScores = [...ghinData.scores].sort((a, b) => new Date(a.played_at).getTime() - new Date(b.played_at).getTime());
    ...
    sortedScores.forEach((score, index) => {
        ...
        if (index % 2 !== 0 && index !== sortedScores.length - 1)
            return;
    ...
    rounds.forEach(round => {
        ...
        if (round.holes.length === 18 && perf.holeAverages) {
            round.holes.forEach((hole, idx) => {
                ...
                if (hole.score < holeAvg.par)
                    holeAvg.birdiePlus += (1 / perf.rounds) * 100;
                if (hole.score <= holeAvg.par)
                    holeAvg.parOrBetter += (1 / perf.rounds) * 100;
            });
        }
    });
    // Calculate shot statistics from rounds with hole details
    const roundsWithHoles = rounds.filter(r => r.holes.length > 0);
    const totalFairways = roundsWithHoles.reduce((sum, r) => sum + r.holes.filter(h => h.fairwayHit !== undefined && h.fairwayHit !== null).length, 0);
    const fairwaysHit = roundsWithHoles.reduce((sum, r) => sum + r.holes.filter(h => h.fairwayHit === true).length, 0);
    ...
    const greensHit = roundsWithHoles.reduce((sum, r) => sum + r.holes.filter(h => h.greenInRegulation === true).length, 0);
    const totalPutts = roundsWithHoles.reduce((sum, r) => sum + r.holes.reduce((pSum, h) => pSum + (h.putts || 0), 0), 0);
    const girPutts = roundsWithHoles.reduce((sum, r) => sum + r.holes.filter(h => h.greenInRegulation).reduce((pSum, h) => pSum + (h.putts || 0), 0), 0);
    const missedGIRUpDowns = roundsWithHoles.reduce((sum, r) => sum + r.holes.filter(h => !h.greenInRegulation && h.score <= h.par).length, 0);
    const missedGIRTotal = roundsWithHoles.reduce((sum, r) => sum + r.holes.filter(h => !h.greenInRegulation).length, 0);
    ...
    roundsWithHoles.forEach(round => {
        round.holes.forEach(hole => {
            ...
            if (!stats)
                return; // Skip if not par 3, 4, or 5
            stats.totalHoles++;
            ...
            if (scoreToPar <= -1)
                ...
                stats.doublePlus++; // Double bogey or worse
        });
    });
    // Convert to HoleTypePerformance array
    const holeTypePerformance = Array.from(holeTypeStats.entries())
    ...
    return {
        ...
        coursePerformance: Array.from(coursePerformanceMap.values())
            ...
            roundsList: course.roundsList.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()),
        ...
        goalsData: calculateGoalsData(ghinHandicapHistory[ghinHandicapHistory.length - 1].handicapIndex, ghinHandicapHistory)
    };
}
// Export a promise that resolves to the golf data
let golfDataPromise = null;
export function getGolfData() {
    if (!golfDataPromise) {
